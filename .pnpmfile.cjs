...
const {
  getPnpmCommand,
  getPnpmPackages,
} = require('./infra/devx-and-repo/repo-shell-scripts/pnpmfile-helper.cjs');
...
const patchPackageJsonAndAddDevtoolsDependencies = (packageJson) => {
  if (
    ...
    WORKSPACE_PACKAGES.some(
      (workspacePackageInfo) => packageJson.name === workspacePackageInfo.name,
    )
  ) {
    ORIGINAL_WORKSPACE_PACKAGE_JSON_MAP.set(packageJson.name, {
      ...packageJson,
    });
...
const IS_RELEASING_PACKAGE =
  pnpmCommand === 'pack' || pnpmCommand === 'publish';

if (!IS_RELEASING_PACKAGE) {
  const originalJsonParse = JSON.parse.bind(JSON);
  const originalJsonStringify = JSON.stringify.bind(JSON);
  JSON.parse = (text, ...otherArgs) =>
    patchPackageJsonAndAddDevtoolsDependencies(
      originalJsonParse(text, ...otherArgs),
    );
  JSON.stringify = (obj, ...otherArgs) => {
    let finalObject = obj;
    const workspacePackageOriginalJson =
      ORIGINAL_WORKSPACE_PACKAGE_JSON_MAP.get(obj.name);
    if (
      workspacePackageOriginalJson &&
      ((workspacePackageOriginalJson.devDependencies &&
        !finalObject.devDependencies) ||
        (workspacePackageOriginalJson.devtoolsDependencies &&
          !finalObject.devtoolsDependencies))
    ) {
      finalObject = { ...obj };
      ...
      if (workspacePackageOriginalJson.devDependencies) {
        ...
        Object.keys(workspacePackageOriginalJson.devDependencies).forEach(
          (dependencyName) => {
            if (finalObject.dependencies[dependencyName]) {
              finalObject.devDependencies[dependencyName] =
                finalObject.dependencies[dependencyName];
